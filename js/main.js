document.addEventListener("DOMContentLoaded", () => {
  ...
  function winOne(){
    const winOne = window.localStorage.getItem("winOne") || 0;
    window.localStorage.setItem("winOne", Number(winOne) + 1);
    console.log(winOne)
  }

  function winTwo(){
    const winTwo = window.localStorage.getItem("winTwo") || 0;
    window.localStorage.setItem("winTwo", Number(winTwo) + 1);
    console.log(winTwo)
  }

  function winThree(){
    const winThree = window.localStorage.getItem("winThree") || 0;
    window.localStorage.setItem("winThree", Number(winThree) + 1);
    console.log(winThree)
  }

  function winFour(){
    const winFour = window.localStorage.getItem("winFour") || 0;
    window.localStorage.setItem("winFour", Number(winFour) + 1);
    console.log(winFour)
  }

  function winFive(){
    const winFive = window.localStorage.getItem("winFive") || 0;
    window.localStorage.setItem("winFive", Number(winFive) + 1);
  }

  function winSix(){
    const winSix = window.localStorage.getItem("winSix") || 0;
    window.localStorage.setItem("winSix", Number(winSix) + 1);
  }
      ...
      const res = await fetch(
        `https://wordsapiv1.p.rapidapi.com/words/${guessedWord.toLowerCase()}`,
        {
          ...
          headers: {
            ...
            "x-rapidapi-key": "5de16b936bmshf59586ad815db8bp139302jsnbe4509ff5316",
          },
        }
      );
      ...
      if (guessedWord === currentWord) {
        setTimeout(() => {
          ...
          if (okSelected) {
            ...
            if(guessedWordCount === 3){
              console.log(guessedWordCount);
              winThree()
            }
            if(guessedWordCount === 4){
              console.log(guessedWordCount);
              winFour()
            }
  ...
  function initSettingModal() {
   const modal = document.getElementById("setting-modal");
    ...
    const span = document.getElementById("close-setting");
    ...
    var cb = document.getElementById("switch-cb");
    var scheme = 0;
    cb.onclick = function(){
    if(scheme == 0){
    document.getElementById("css").href="css/styles1.css";
    scheme = 1;
    }
    else{
  ...
  var countDownDate = new Date("Jan 5, 2024 24:00:00").getTime();
  ...
  function updateStatsModal() {
    ...
    const winOne = window.localStorage.getItem("winOne");
    const winTwo = window.localStorage.getItem("winTwo");
    const winThree = window.localStorage.getItem("winThree");
    const winFour = window.localStorage.getItem("winFour");
    const winFive = window.localStorage.getItem("winFive");
    const winSix = window.localStorage.getItem("winSix");
    ...
    var config = {
      ...
      data: {
        ...
        datasets: [{
          label: "success time",
          data: [winOne, winTwo, winThree, winFour, winFive, winSix],
          backgroundColor: [
            "rgba(255, 99, 132, 0.2)",
            "rgba(54, 162, 235, 0.2)",
            "rgba(75, 192, 192, 0.2)",
